...
class TestRunner:
    ...
    def __init__(self, base_url: str = "http://localhost:8000"):
        ...
        self.skip_test = False  # 标记是否跳过当前测试
    ...
    def check_api_response(self, response, test_name: str = "API测试") -> Dict[str, Any]:
    ...
    def test_api_endpoint_exists(self):
        ...
        assert response.status_code == 200, \
            f"OPTIONS请求失败: {response.status_code}"
    ...
    def test_missing_prompt_parameter(self):
        ...
        assert result['error_code'] == 'INVALID_PARAMS', f"错误码不正确: {result['error_code']}"
    ...
    def test_invalid_json_format(self):
        ...
        result = self.assert_response(response, 400, ['success', 'error_code', 'error'])
        assert result['success'] == False, "应该返回失败"
        assert result['error_code'] == 'INVALID_JSON', f"错误码不正确: {result['error_code']}"
    ...
    def test_real_success_case(self):
        ...
        print(f"  建议文本: {text[:100]}..." if len(text) > 100 else f"  建议文本: {text}")
    ...
    def test_many_includes(self):
        ...
        includes = [f"#include <header{i}.h>" for i in range(20)]  # 超过MAX_INCLUDES(10)
        data = {
            "prompt": "int main() {\n    ",
            "suffix": "\n}",
            "includes": includes
    ...
    def test_many_functions(self):
        ...
        functions = [{"name": f"func{i}", "signature": f"void func{i}()"} for i in range(10)]  # 超过MAX_FUNCTIONS(5)
        data = {
            "prompt": "int main() {\n    ",
            "suffix": "\n}",
    ...
    def test_cors_headers(self):
        ...
        assert 'POST' in headers.get('Access-Control-Allow-Methods', ''), "CORS方法头不正确"
        assert 'Content-Type' in headers.get('Access-Control-Allow-Headers', ''), "CORS头不正确"
